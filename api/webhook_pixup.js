...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Busca transação pelo external_id (você deve salvar o external_id ao criar o PIX!)
    const { data: transacao, error: tError } = await supabase
      .from('transacoes')
      .select('*')
      .eq('id', body.external_id) // Se seu external_id não é o id da transação, ajuste aqui
      .single();
